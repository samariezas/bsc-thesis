...
Groverio algoritmas sprendžia nestruktūrizuotos paieškos problemą: duotoje $N$ elementų erdvėje reikia rasti pažymėtą elementą. Klasikinėje paieškoje blogiausiu atveju reikia patikrinti $O(N)$ elementų, o Groverio algoritmas pažymėtą elementą randa naudojant apie $O(\sqrt{N})$ oracle užklausų \cite{grover_1996,nielsen_chuang_2010}.
...
Ši formulė matoma ir darbo programoje \texttt{grover.c}: iteracijų skaičius apskaičiuojamas kaip \texttt{pi/4*sqrt(1 << qpu\_getregwidth(reg))}. Kadangi rezultatas priskiriamas sveikajam skaičiui, gaunama apytikslė, žemyn suapvalinta iteracijų reikšmė.
