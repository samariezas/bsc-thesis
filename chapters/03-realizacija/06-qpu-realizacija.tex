...
Realizuotos QPU instrukcijos matomos \ref{tab:qpu-realizacijos-rysys} lentelėje. C sąsaja naudojami registrai ir \texttt{funct7} laukai išvardinti \ref{chap:priedas-qpu-instr} priede.
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \hline
        \texttt{Q.NEWCTX} & naujo QPU konteksto sukūrimas, leidžiama vykdyti tik S ir M režimuose \\
        \hline
        \texttt{Q.FREECTX} & QPU konteksto išlaisvinimas, leidžiama vykdyti tik S ir M režimuose \\
        \hline
        \texttt{Q.NEWREG} & naujo \texttt{libquantum} registro sukūrimas \\
        \hline
        \texttt{Q.CNOT} & \textit{CNOT} vartų taikymas \\
        \hline
        \texttt{Q.TOFFOLI} & \textit{Toffoli} vartų taikymas \\
        \hline
...
Kai kurioms operacijoms reikalingas tarpinis C sluoksnis. Kadangi Zig nepalaiko sąveikos su C kompleksiniais skaičiais, kai kurioms \texttt{libquantum} operacijoms reikalingas papildomas sluoksnis kuris priima realią ir menamą amplitudės dalį kaip paprastas slankiojo kablelio reikšmes.

QPU instrukcijų operandų išdėstymas iš esmės atitinka R tipo instrukcijų laukus, tačiau kai kurioms operacijoms prireikė papildomų konvencijų. Pavyzdžiui, \texttt{Q.TOFFOLI} turi tris kubitų argumentus ir registro rankeną, todėl antras kontrolinis kubitas perduodamas per sutartą papildomą bendrosios paskirties registrą. \texttt{Q.GETREGNODE} taip pat naudoja papildomus įvesties registrus, o rezultatą grąžina per kelis išvesties registrus. Vartotojo programoje šios instrukcijos įterpiamos per asemblerio \texttt{.insn r} sintaksę, kuri užtikrina kad papildomų įvesties registrų būsena bus užsaugoma.
...
\subsection{QPU kodo kompiliavimas su TCC}

Vartotojo erdvėje pateikiamas \texttt{tcc} kompiliatorius ir \texttt{qpu.h} failas, sukuriantis patogią C sąsają su instrukcijomis. Nors \texttt{tcc} palaiko asemblerio sintaksę, šis kompiliatorius neleidžia kurti vartotojo instrukcijų \texttt{GCC} stiliumi, naudojant \texttt{.insn r} sintaksę instrukcijoms su pasirinktiniais \texttt{opcode}, \texttt{funct3} ir \texttt{funct7} laukais. Dėl to buvo sukurtas papildomas \texttt{libqpu} sluoksnis, kompiliuojamas \texttt{GCC} ir įkeliamas į initramfs. Tuomet \texttt{tcc} gali sėkmingai susieti vartotojo kodą ir naudoti QPU instrukcijas. Pavyzdinio kvantinio skaičiavimo kodas pateiktas \ref{chap:priedas-paleidimas} priede.
